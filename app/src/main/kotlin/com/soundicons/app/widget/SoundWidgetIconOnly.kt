...
private const val TAG = "SoundWidgetIconOnly"

/**
 * AppWidgetProvider for the "Icon Only" style.
 *
 * A single ImageView fills the entire widget at any size — no label.
 * When the user resizes the widget, [onAppWidgetOptionsChanged] fires and
 * we rebuild the bitmap at the new size so it stays sharp.
 *
 * All rendering delegates to [WidgetRemoteViewsHelper] which contains the
 * actual layout/bitmap logic.
 */
class SoundWidgetIconOnly : AppWidgetProvider() {
                ...
                if (icon != null && mapping != null) {
                    ...
                    appWidgetManager.updateAppWidget(
                        ...
                        WidgetRemoteViewsHelper.buildPlaceholder(
                            context, com.soundicons.app.data.model.WidgetMapping.DisplayMode.ICON_ONLY
                        )
                    )
                }
            }
    ...
    /** Called every time the user drags to resize — re-renders at new size */
    ...
    companion object {
        /** Called by [WidgetConfigureActivity] and [HomeViewModel] after icon changes */
        ...
        /** Refresh all Icon Only widgets — called on boot or after bulk icon edits */
        fun refreshAll(context: Context) {
            val repo    = WidgetEntryPoint.resolve(context).widgetRepository()
            val manager = AppWidgetManager.getInstance(context)
            val ids     = manager.getAppWidgetIds(
                ComponentName(context, SoundWidgetIconOnly::class.java)
            )
